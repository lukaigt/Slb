...
## Overview
This project is a self-learning perpetual futures scalping bot on Solana mainnet using Drift Protocol. The bot trades 23 coins simultaneously: SOL, BTC, ETH, DOGE, AVAX, LINK, ADA, DOT, ATOM, NEAR, SUI, LTC, XMR, ALGO, HBAR, TRX, RENDER, APT, UNI, ARB, OP, FIL, POL. v19.0 "Perfect Bot" upgrades: (1) Wilson-lower-bound WR ≥50% + expected-value gate (both must pass to enter pattern-match); (2) exponential recency decay (14-day half-life) × similarity weighting on k-NN neighbors; (3) weighted feature similarity (trend=3×, ATR/EMA=2×); (4) trailing stops — breakeven lock at 50% TP, trail peak-0.25% at 75% TP, canonical `threshold = -sl` so negative SL values correctly lock profit; (5) strict 5m+15m EMA trend alignment required for entries; (6) hour-of-day filter (blocks entries when UTC hour has ≥20 samples & WR <40%); (7) tick-driven SL/TP via Kraken WebSocket onPrice → fastTickCheck (real-time exits, not 2s poll); (8) per-symbol `closeInFlight` mutex preventing duplicate-close race between poll and tick paths. v18.3: Historical pattern backfill (`backfill.js`). v18.2: 23 coins via Kraken, relaxed learning gates. Pattern Memory Engine: 38-dim fingerprints, k-NN matching, persistent data/ storage. Dashboard server-rendered with manual refresh — now surfaces Wilson WR gate, EV gate, half-life, hour heatmap with block-borders, 5m/15m trend column, trailing-stop stage + peak. Data source switchable between Kraken and Drift DLOB via `DATA_SOURCE`.
